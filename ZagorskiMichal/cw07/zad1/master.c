...
int main(int argc, char** argv){
    handle(argc, != 4, "Wrong number of arguments\n"
        "There should be <consumer count> <producer count> <file for shared memory> <file for semaphore>", 1);
    int mem = -1;
    key_t shm_key;
    handle((shm_key = ftok(argv[3], SHM)), == -1, "Can't get key for shared memory", 1);
    handle((mem = shmget(shm_key, SIZE + 2, IPC_CREAT | 0666)), 
    ...
    int index = 0;
    ushort* sem_vals = (ushort* )calloc(SIZE+3, sizeof(ushort));
    ...
    handle(semctl(semaphores, 0, SETALL, sem_begin), == -1, "Can't set semaphores begin state", 1);
    if(sem_vals != NULL){
        free(sem_vals);
    }
